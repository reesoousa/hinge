...
The renderer keeps its last frame across a pause, so a fold starts on retained content while the new stream spins up rather than waiting on capture. Building that stream is deliberately kept off the main actor. The display link that drives drawing is scheduled on the main run loop, so constructing the stream and attaching its output there stole frames from the first moments of a fold, which is exactly when the resume happens. The content filter and stream configuration are cached, which lets a resume skip shareable content enumeration and the blur warm up. Included windows are refreshed just before a suspend rather than after a resume. Enumerating shareable content is an expensive call and updating the content filter disturbs capture, so doing either as the fold begins produced a visible stutter. At rest there is time to spare. Turning the option off keeps a single stream running for the whole session, which is the earlier behavior.
